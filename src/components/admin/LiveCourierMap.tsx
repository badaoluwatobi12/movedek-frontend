...
function FitLiveCouriers({
  ...
  useEffect(() => {
    ...
    if (couriers.length === 1) {
      ...
      if (courier)
        map.flyTo([courier.latitude, courier.longitude], 15, { duration: 0.8 });
      return;
    }
    map.fitBounds(
      L.latLngBounds(
        couriers.map((courier) => [courier.latitude, courier.longitude]),
      ),
      {
        padding: [48, 48],
        maxZoom: 15,
...
export function LiveCourierMap({ className }: { className?: string }) {
    ...
    try {
      ...
      setError(
        requestError instanceof Error
          ...
          : "Unable to load courier locations.",
      );
    } finally {
      setLoading(false);
    }
    ...
    socket.on("courier:location", (location: LiveCourier) => {
      setCouriers((current) => {
        const exists = current.some(
          (candidate) => candidate.courier_id === location.courier_id,
        );
        return exists
          ? current.map((candidate) =>
              candidate.courier_id === location.courier_id
                ? location
                : candidate,
            )
          : [location, ...current];
  ...
  const liveCount = useMemo(
    () => couriers.filter((courier) => !courier.stale).length,
    [couriers],
  );
  ...
  return (
    <div
      className={`relative overflow-hidden rounded-2xl border border-border bg-muted/20 ${className ?? ""}`}
    >
      <MapContainer
        center={LAGOS_CENTER}
        zoom={12}
        ...
        {couriers.map((courier) => (
          <div key={courier.courier_id}>
            ...
            <Marker
              ...
              icon={courierIcon(courier)}
            >
              <Popup minWidth={240}>
                <div className="space-y-2 text-sm">
                  <div>
                    <p className="font-semibold">
                      {courier.full_name ?? "Courier"}
                    </p>
                  ...
                  <div className="grid grid-cols-2 gap-2 text-xs">
                    ...
                    <span>
                      Accuracy:{" "}
                      {courier.accuracy == null
                        ? "—"
                        : `±${Math.round(courier.accuracy)}m`}
                    </span>
                  </div>
                  {courier.active_delivery ? (
                    <div className="rounded-md bg-muted p-2 text-xs">
                      <p className="font-medium">
                        Delivery {courier.active_delivery.id}
                      </p>
                      <p className="capitalize text-muted-foreground">
                        {courier.active_delivery.status.replaceAll("_", " ")}
                      </p>
                    </div>
                  ) : (
                    <p className="text-xs text-muted-foreground">
                  ...
                  >
                    {courier.stale ? "Location stale" : "Live"} · updated{" "}
                    {ageLabel(courier.age_seconds)}
                  </p>
                </div>
              </Popup>
            </Marker>
      ...
      <div className="absolute right-3 top-3 z-[1000] flex gap-2">
        <button
          ...
          className="rounded-full bg-background/95 p-2 shadow-md"
          title="Toggle automatic map fitting"
        >
          {autoFit ? (
        ...
        <div className="pointer-events-none absolute inset-0 z-[900] flex items-center justify-center p-6">
          <div className="max-w-md rounded-2xl border bg-background/95 p-5 text-center shadow-xl backdrop-blur">
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              An approved courier must be online, grant location permission, and
              open an active delivery in the MoveDek mobile app.
            </p>
          </div>
        </div>
      ) : null}
