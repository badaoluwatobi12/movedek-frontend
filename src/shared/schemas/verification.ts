...
export const courierVerificationSchema = z.object({
  ...
  service_zones: z.string().trim().min(2, "Add at least one preferred work zone."),
  vehicle_type: z.string().trim().min(1, "Select a courier type."),
  vehicle_model: z.string().trim().min(1, "Vehicle / movement type is required."),
  ...
  emergency_contact_phone: z.string().trim().min(7, "Emergency contact phone is required."),
  selfie_uploaded: z.boolean(),
  id_uploaded: z.boolean(),
  license_uploaded: z.boolean(),
});

export type CourierVerificationInput = z.infer<typeof courierVerificationSchema>;
