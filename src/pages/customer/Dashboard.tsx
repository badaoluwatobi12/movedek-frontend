...
import { Package, Wallet, MapPin, CheckCircle2, Plus, ArrowRight, Star } from "lucide-react";
...
export default function CustomerDashboard() {
  ...
  if (deliveriesQuery.isLoading) return <LoadingState label="Loading your dashboard…" />;
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Here's what's moving today.</p>
        </div>
        <Link to="/app/new">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <StatCard label="Active deliveries" value={String(active.length)} icon={Package} tone="accent" />
        <StatCard label="Completed" value={String(completed.length)} icon={CheckCircle2} tone="success" />
        <StatCard label="Wallet balance" value={naira(wallet?.balance ?? 0)} icon={Wallet} tone="warning" />
        <StatCard label="Saved addresses" value={String(savedAddresses.length)} icon={MapPin} />
      </div>

      <div className="grid gap-6 lg:grid-cols-3">
        <div className="lg:col-span-2 space-y-3">
          <h2 className="font-display text-lg font-semibold text-primary">Active deliveries</h2>
          {active.length === 0 ? (
            <EmptyState
              icon={Package}
              ...
              action={
                <Link to="/app/new">
                  <Button className="accent-gradient text-white">Request delivery</Button>
                </Link>
              }
            />
          ) : (
            active.map((d) => {
              ...
              const cUser = courier && users.find((u) => u.id === courier.user_id);
              return (
                <Link
                  to={`/app/track/${d.id}`}
                  key={d.id}
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <div className="text-xs text-muted-foreground">
                        #{d.id.slice(0, 6).toUpperCase()} · {timeAgo(d.created_at)}
                      </div>
                      <div className="mt-1 font-display font-semibold text-primary">{d.item_name}</div>
                      <div className="mt-1 text-sm text-muted-foreground flex items-center gap-2">
                        <MapPin className="h-3.5 w-3.5" /> {d.pickup_address.split(",")[0]} →{" "}
                  ...
                  <div className="mt-4 flex items-center justify-between rounded-xl bg-muted/40 p-3">
                    {courier && cUser ? (
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <div className="text-sm font-medium">{cUser.full_name}</div>
                      ...
                      <span className="text-sm text-muted-foreground">Searching for nearby courier…</span>
                    )}
        ...
        <div className="space-y-3">
          <h2 className="font-display text-lg font-semibold text-primary">Recent history</h2>
          {completed.slice(0, 4).map((d) => (
            <div key={d.id} className="card-soft p-4">
              ...
              <div className="text-xs text-muted-foreground">{naira(d.price)}</div>
            </div>
          ))}
          <Link
            to="/app/history"
