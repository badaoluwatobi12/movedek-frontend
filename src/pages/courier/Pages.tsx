...
export function CourierEarnings() {
  ...
  const payoutTransactions = wallet
    ? walletTx.filter(
        (tx) =>
          tx.wallet_id === wallet.id && tx.type === "credit" && tx.description.startsWith("Courier payout"),
      )
    : [];
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <h1 className="font-display text-2xl font-bold text-primary">Earnings</h1>
          ...
          <TableBody>
            ...
            {payoutTransactions.map((tx) => (
              <TableRow key={tx.id}>
                ...
                <TableCell className="text-success">+{naira(tx.amount)}</TableCell>
                <TableCell>
                  <PaymentBadge
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3">
        <h1 className="font-display text-2xl font-bold text-primary">Ratings</h1>
          ...
          <div key={r.id} className="card-elevated p-4">
            <div className="flex items-center justify-between">
              <div className="font-medium text-primary">
                {users.find((u) => u.id === r.from_user_id)?.full_name ?? "Customer"}
              </div>
